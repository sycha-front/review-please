...
- [팬플러스, 4월 15일] A PR입니다. (PR 링크)
  - 날짜 파싱, PR 링크 바로 연결 가능
...
- `.dmg` 파일을 다운로드받아 실행했는데 손상되었다고 나올 때
  - 터미널에 `xattr -dr com.apple.quarantine "/Applications/review-please.app"` 실행
    (애플의 커스텀 앱 제한 우회)
- 갑자기 정보 불러오기가 멈춘 것처럼 보일 때
  - 트레이 아이콘을 우클릭 - Clear Local DB Data로 초기화 후 다시 시도해보세요.
...
- Tauri 로직 부분에 개선점이 많을 수 있습니다. 컨트리뷰션 환영합니다!
- 이슈 생성 > PR
