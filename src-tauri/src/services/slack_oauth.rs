...
#[cfg(test)]
mod tests {
    use super::{parse_poll_response, SlackOAuthSessionState};

    #[test]
    fn parses_pending_session() {
        let state =
            parse_poll_response(br#"{"status":"pending","expiresAt":"2026-03-31T12:00:00Z"}"#)
                .expect("pending session");

        assert_eq!(
            state,
            SlackOAuthSessionState::Pending {
                expires_at: Some("2026-03-31T12:00:00Z".to_string())
            }
        );
    }
    ...
    #[test]
    fn parses_failed_session() {
        let state =
            parse_poll_response(br#"{"status":"failed","error":"oauth_denied"}"#).expect("failed");

        assert_eq!(
            state,
            SlackOAuthSessionState::Failed {
                error: "oauth_denied".to_string()
            }
        );
    }
    ...
    fn rejects_unknown_status() {
        let error = parse_poll_response(br#"{"status":"wat"}"#).expect_err("unknown status");
        assert!(error
            .to_string()
            .contains("unsupported Slack OAuth session status"));
    }
}
